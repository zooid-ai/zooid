...
export class ZooidClient {
  ...
  async updateServerMeta(options: UpdateServerMetaOptions): Promise<ServerIdentity> {
    return this.request<ServerIdentity>('PUT', '/api/v1/server', options);
  }

  /** List all channels via `GET /api/v1/channels`. */
  async listChannels(): Promise<ChannelInfo[]> {
    const res = await this.request<{ channels: ChannelInfo[] }>('GET', '/api/v1/channels');
    return res.channels;
  }
  ...
  async addPublisher(channelId: string, name: string): Promise<PublisherResult> {
  ...
  async publish(channelId: string, options: PublishOptions): Promise<ZooidEvent> {
    const body: Record<string, unknown> = { data: options.data };
    if (options.type !== undefined) {
      body.type = options.type;
    }
    return this.request<ZooidEvent>('POST', `/api/v1/channels/${channelId}/events`, body);
  }

  /** Publish multiple events in a single request. Requires a publish-scoped token. */
  async publishBatch(channelId: string, events: PublishOptions[]): Promise<ZooidEvent[]> {
    const body = {
      events: events.map((e) => {
  ...
  async registerWebhook(
    ...
    return this.request<WebhookResult>('POST', `/api/v1/channels/${channelId}/webhooks`, {
      url,
      ...options,
    });
  }
