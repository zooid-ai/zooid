...
describe('ZooidClient.subscribe() — poll mode', () => {
  it('calls callback with events from initial poll', async () => {
    ...
    const unsub = await client.subscribe('signals', callback, { mode: 'poll', interval: 5000 });
  ...
  it('polls periodically at the given interval', async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        events: [{ id: 'e2', type: 'x', data: '{}', created_at: '2026-01-01T00:00:05Z' }],
        cursor: 'e2',
        has_more: false,
      }),
    );
  ...
  it('uses cursor from previous poll for subsequent requests', async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        events: [{ id: 'e1', type: 'x', data: '{}', created_at: '2026-01-01T00:00:00Z' }],
        cursor: 'e1',
        has_more: false,
      }),
    );
  ...
  it('continues polling even if a poll errors', async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        events: [{ id: 'e1', type: 'x', data: '{}', created_at: '2026-01-01T00:00:10Z' }],
        cursor: 'e1',
        has_more: false,
      }),
    );
...
describe('ZooidClient.subscribe() — ws mode', () => {
  ...
  it('builds correct WS URL from https server', async () => {
    const client = new ZooidClient({ server: 'https://example.com', token: 'tok123' });
    const callback = vi.fn();

    const promise = client.subscribe('signals', callback, { mode: 'ws', type: 'alert' });

    expect(MockWebSocket.instances).toHaveLength(1);
    const ws = MockWebSocket.instances[0];
    expect(ws.url).toBe('wss://example.com/api/v1/channels/signals/ws?token=tok123&types=alert');
  ...
  it('delivers WS messages to callback', async () => {
    ...
    const event = { id: 'e1', type: 'signal', data: '{"v":1}', created_at: '2026-01-01T00:00:00Z' };
    ws.simulateMessage(JSON.stringify(event));

    expect(callback).toHaveBeenCalledTimes(1);
    expect(callback).toHaveBeenCalledWith(expect.objectContaining({ id: 'e1', type: 'signal' }));
  });
