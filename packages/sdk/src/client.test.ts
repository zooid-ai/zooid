...
describe('ZooidClient', () => {
  describe('construction', () => {
    ...
    it('accepts an optional token', () => {
      const client = new ZooidClient({ server: 'https://example.com', token: 'tok' });
  ...
  describe('addPublisher()', () => {
    it('sends POST /api/v1/channels/:id/publishers', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        jsonResponse({ id: 'pub-1', name: 'whale-bot', publish_token: 'pt' }, 201),
      );
  ...
  describe('publish()', () => {
    it('sends POST /api/v1/channels/:id/events with event data', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        jsonResponse(
          { id: 'evt-1', channel_id: 'signals', type: 'alert', data: '{"v":1}' },
          201,
        ),
      );
    ...
    it('sends event without type when not provided', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        jsonResponse({ id: 'evt-2', channel_id: 'ch', type: null, data: '{}' }, 201),
      );
  ...
  describe('registerWebhook()', () => {
    it('sends POST /api/v1/channels/:id/webhooks', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        jsonResponse(
          { id: 'wh-1', channel_id: 'signals', url: 'https://hook.example.com' },
          201,
        ),
      );

      const wh = await client.registerWebhook('signals', 'https://hook.example.com', {
        event_types: ['alert'],
        ttl_seconds: 3600,
      });
