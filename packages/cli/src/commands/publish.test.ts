...
function writeConfig(channelTokens: Record<string, string> = {}) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ server: 'https://test.workers.dev', admin_token: 'admin-jwt', channels }),
  );
}
...
describe('publish command', () => {
  it('publishes an event with type and data', async () => {
    ...
    const result = await runPublish('signals', { type: 'alert', data: '{"v":1}' });
  ...
  it('publishes from a JSON file', async () => {
    ...
    fs.writeFileSync(filePath, JSON.stringify({ type: 'test', data: { a: 1 } }));

    mockClient.publish.mockResolvedValueOnce({
      id: 'evt-2', channel_id: 'signals', type: 'test', data: '{"a":1}',
    });
  ...
  it('publishes data-only event without type', async () => {
    writeConfig({ signals: 'pub-tok' });
    mockClient.publish.mockResolvedValueOnce({
      id: 'evt-3', channel_id: 'signals', type: null, data: '{"hello":"world"}',
    });
