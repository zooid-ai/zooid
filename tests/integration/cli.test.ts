...
describe('CLI Integration Tests', () => {
  ...
  async function makeAdminClient() {
    ...
    return new ZooidClient({ server: 'https://test.local', token, fetch: testFetch });
  }
  ...
  describe('channel create -> list -> add-publisher (full flow)', () => {
    it('creates a channel via handler, lists it, adds a publisher', async () => {
      const { runChannelCreate, runChannelList, runChannelAddPublisher } = await import('../../packages/cli/src/commands/channel');
      const client = await makeAdminClient();

      // Create
      const created = await runChannelCreate('int-test', {
  ...
  describe('publish -> poll (full flow)', () => {
    it('publishes an event and polls it back via handlers', async () => {
      const { runChannelCreate } = await import('../../packages/cli/src/commands/channel');
      const { runPublish } = await import('../../packages/cli/src/commands/publish');

      const adminClient = await makeAdminClient();

      // Create channel
      const ch = await runChannelCreate('pub-int', { name: 'Pub Int', public: true }, adminClient);

      // Publish with the publish token
      const pubClient = makeClient(ch.publish_token);
      const event = await runPublish('pub-int', {
        type: 'signal',
  ...
  describe('publish data-only (full flow)', () => {
    it('publishes data without type via handler', async () => {
      ...
      const adminClient = await makeAdminClient();
      const ch = await runChannelCreate('file-int', { name: 'File Int', public: true }, adminClient);

      const pubClient = makeClient(ch.publish_token);
      const event = await runPublish('file-int', {
        data: '{"hello":"world"}',
      }, pubClient);
      expect(event.id).toBeTruthy();
    });
  ...
  describe('status (full flow)', () => {
    it('fetches server discovery and identity via handler', async () => {
      const { runStatus } = await import('../../packages/cli/src/commands/status');
  ...
  describe('subscribe webhook (full flow)', () => {
    it('registers a webhook via handler', async () => {
      const { runChannelCreate } = await import('../../packages/cli/src/commands/channel');
      const { runSubscribeWebhook } = await import('../../packages/cli/src/commands/subscribe');

      const adminClient = await makeAdminClient();
      await runChannelCreate('wh-int', { name: 'WH Int', public: true }, adminClient);

      const result = await runSubscribeWebhook('wh-int', 'https://example.com/hook', adminClient);
  ...
  describe('server meta (full flow)', () => {
    it('gets and sets server metadata via handlers', async () => {
      const { runServerGet, runServerSet } = await import('../../packages/cli/src/commands/server');
      const adminClient = await makeAdminClient();
      const reader = makeClient();
